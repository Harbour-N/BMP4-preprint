---
title: Simulated BMP4 and RT
description: Simulate the model with parameter values drawn from real patient data and also virtually simulate BMP4 at time of resection and RT
authors:
...
# Virtual clinical trial: resection + BMP4 + RT

Simulate the model for a cohort of virtual patients who undergo our novel proposed treatment of BMP4 and RT following resection.
```{python}
...
```{python}
#| label: fig-RT_BMP4_KM
#|fig-cap: Simulated survival curves for BMP4 + RT
...
plt.ylabel("Survival")
plt.xlabel("Time (day)")
plt.title(r"Simulated survival BMP4 + RT")
plt.legend([r"Virtual control", r"Simulated BMP4 $\psi = 2$",
            r"Simulated BMP4 $\psi = 5$", r"Simulated BMP4 $\psi = 50$", r"Simulated BMP4 $\psi = 100$", "Real"])
plt.show()
```

plot only a subset of the survival curves for clarity

```{python}
#| label: fig-RT_BMP4_KM_SMB
#|fig-cap: Simulated survival curves for BMP4 + RT
#| fig-alt: Alternate text
...
plt.ylabel("Survival")
plt.xlabel("Time (day)")
plt.title(r"Simulated survival")
plt.legend([r"Virtual control", r"Simulated BMP4 $\psi = 2$", r"Simulated BMP4 $\psi = 100$"])
plt.show()

```

## Example tumour trajectories: Resection + RT vs Resection + RT + BMP4

Plot representative model outputs for a cohort of virtual patients who undergo standard of care treatment, resection followed by radiotherapy (entry 0 in saved data); and for Resection + BMP4 + RT for a fixed psi value of 100 (entry 4 in saved data). 

```{python}
#| label: fig-effect_of_BMP4_sims
#| fig-cap: Five example simulations showing tumour growth, detection, resection and the effect of radiotherapy along (solid curves) and radiotherapy + BMP4 (dashed curves). BMP4 delivered via mesenchymal stem cells at the time of resection increases the rate of differentiation of stem cells (GSCs) to TCs, so the stem cell fraction drops making residual tumour more susceptible to radiotherapy, prolonging survival. 
